...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
    ...
    api.sendMessage(` ⏱️ | جاري تنزيل الفيديو \n-----------\n${body}\n-----------\nهذا قد يستغرق مدة من الأفضل الإنتظار !`, event.threadID, (err, info) =>
  ...
  try {
    ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
      ...
      ytdl(handleReply.link[event.body - 1])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage(' ⚠️ |تعذر إرسال الفيديو لأن حجمه أكبر من 25 ميغابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
        ...
        .on("error", (error) => api.sendMessage(` ⏱️ |حدثت مشكلة أثناء معالجة الطلب، خطأ: \n${error}`, event.threadID, event.messageID));
    });
  }
  catch {
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (args.length == 0 || !args) return api.sendMessage(' ⚠️ |لا يمكن ترك البحث فارغا!', event.threadID, event.messageID);
  ...
  if (urlValid) {
    try {
      ...
      ytdl(args[0])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400) return api.sendMessage(' 🚫 |تعذر إرسال الملف لأن حجمه أكبر من 25 ميغابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
    ...
    try {
      ...
      var body = `🎼 هناك ${link.length} تطابق ما كنت تبحث عنه :\n👇👇👇👇👇\n${msg}\n أرجوك قم بالرد على الرسالة أعلاه برقم الفيديو من أجل تنزيله و مشاهدته`
      ...
      var body = `🎼 هناك ${link.length} تطابق نتيحة بحثك :\n👇👇👇👇👇\n${msg}\nأرجوك قم بالرد على الرسالة أعلاه برقم الفيديو من أجل تنزيله و مشاهدته`
      return api.sendMessage({ attachment: imgthumnail, body: body }, event.threadID, (error, info) => global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: event.senderID,
